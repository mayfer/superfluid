...
//				camera.applyMatrix( camera_rotation );
                ...
                var rawtime = clock.getElapsedTime();
                time += delta * Math.abs(Math.sin(rawtime));

                var mod_time = rawtime % Math.PI
                var step = 1 + Math.floor( (rawtime - mod_time) / (Math.PI) )

                var freq = 1;
                var progress = step * (Math.sin( mod_time/2));
